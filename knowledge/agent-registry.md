...
## Единая таблица «Авитолог — объекты» — общий рабочий процесс трёх агентов

Google Sheets, spreadsheetId `1mgrnPiPY3oJJJQOqAzgRNKWuj4u41OLxLggNRyOUFdM` (создан через Google Drive connector, три вкладки: Объекты/Конкуренты/Статистика). Полная схема — `AI Avitolog/knowledge/shared-sheets.md`, зеркальные копии у `AI Marketing Strategist`/`AI Legal Constructor` под свою часть процесса (приёмка CMO / юридическая проверка). Статус объекта: `Черновик → В работе → Анализ готов → У CMO → Согласование застройщика → У Юриста → Одобрено → Опубликовано`.

| Компонент | Статус |
|---|---|
| Сама таблица (создание, схема, заморозка заголовков) | ✅ создана и проверена (`get_values` подтвердил заголовки) |
| AI Avitolog (`market-analyst.md`, `avito-listing-packaging/SKILL.md`, `market-recon/SKILL.md`) | ✅ переписаны и закоммичены |
| AI Marketing Strategist (`delegation-protocol.md` §5) | ✅ добавлено и закоммичено |
| AI Legal Constructor (`CLAUDE.md` §1 п.4) | ✅ добавлено и закоммичено |
| Сквозная проверка вживую (запись → чтение → смена статуса на реальном объекте) | Не проведена — следующий шаг |
| Доступ Google Sheets connector у сессий Авитолога/CMO/Юриста | Не подтверждено — тот же аккаунт, что и у ENGINEER, но требует рестарта их сессий |
