...
| Агент | Статус |
|---|---|
| ~/.claude.json (глобально: context7, firecrawl) | ✅ прямой `node`, не подтверждено внутри реальной сессии Claude Code (только изолированный тест рукопожатия) |
| AI Avitolog (apify) | ✅ прямой `node` — **не подтверждено реальным подключением**, ждёт рестарта сессии и проверки Авитологом |
