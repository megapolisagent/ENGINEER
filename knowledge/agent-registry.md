...
**Не проверено этим же заходом** — `AI Copywriter`, `AI Intelligence` на тот же паттерн (`${VAR}` в любом `.mcp.json`) не прогонялись. Механическая проверка на будущее: `grep -rn '\${[A-Z_]*}' */.mcp.json` по всем известным агентам — если находка есть, реальная переменная ОС существует (`[Environment]::GetEnvironmentVariable`), не считать наличие ключа в `settings.local.json` достаточным доказательством.
