...
Открытые, ещё не решённые развилки — отдельно в `open-questions.md`, не здесь: это не факты текущего состояния, см. `.claude/skills/knowledge-wiki/SKILL.md`.
...
- [[agent-registry]] (project_knowledge) — статус переноса канона (Composability & Tooling Gate, Confirmation Gate, hooks, data-discipline.md консолидация) по каждому известному агенту, с датой последней проверки. Сверяется перед тем, как считать перенос нового правила завершённым (`CLAUDE.md` §2) и перед Canon Sync Gate (`02-execution-gates.md`).
