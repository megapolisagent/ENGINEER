...
**Beta, Claude Opus 5 / Fable 5 / Sonnet 5 / Opus 4.8 / 4.7.** Gives Claude a token ceiling for an agentic loop so it paces itself and finishes gracefully instead of being cut off. Setup, fields, and spend-observation details: `shared/task-budgets.md`.

---

## Provider Clients (Quick Reference)

When targeting Claude on a third-party platform, use that platform's dedicated client class — not the first-party `Anthropic()` client with a `base_url` override. After construction the client exposes the same `messages.create` / `.stream` surface as the first-party SDK. Per-language client construction for **Amazon Bedrock** (Mantle client), **Microsoft Foundry**, and **Google Cloud Vertex AI**: see `shared/provider-clients.md`.
...
**Workload Identity Federation and third-party provider clients** (Bedrock/Foundry/Vertex construction, WIF env vars): see `shared/provider-clients.md`.
