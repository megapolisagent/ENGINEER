...
>
> **Пробел донора**: тело ссылается на `references/experiments.md` и `references/form.md`, но папка `references/` физически отсутствует уже в исходном импорте — файлы не были перенесены. Ссылки оставлены как есть (не выдуман контент взамен), пробел зафиксирован здесь явно.
