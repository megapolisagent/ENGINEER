...
>
> **Пробел донора**: тело ссылается на `references/content-distribution.md` и `references/headless-cms.md`, но папка `references/` физически отсутствует уже в исходном импорте — файлы не были перенесены. Ссылки оставлены как есть (не выдуман контент взамен), пробел зафиксирован здесь явно.
