...
>
> **Пробел донора**: тело ссылается на `references/tool-reference.md` и `references/templates.md`, но папка `references/` физически отсутствует уже в исходном импорте — файлы не были перенесены. Ссылки оставлены как есть (не выдуман контент взамен), пробел зафиксирован здесь явно.
