...
# (?!ир) после "формул" — не считать "формулировать/формулировка/переформулировать"
# (общее слово "фраза/wording") сигналом расчёта: тот же корень, что "формула", разный смысл.
CALC_SIGNAL_RE = re.compile(
    r"(формул(?!ир)[а-я]*|рассчит[а-я]*|вычисл[а-я]*|процент[а-я]*"
